<script>
	...
	async function loadSupplierCategories() {
		try {
			...
			console.error('Error al cargar categorías de proveedores:', error);
			supplierCategories = [];
		}
	}
...
{/if}
<!-- Modal para crear categor�a de proveedor -->
<Dialog bind:open={showCategoryModal} title="Nueva Categor�a de Proveedor">
<div class="space-y-4">
<form on:submit|preventDefault={handleCategorySubmit} class="space-y-4">
<div class="space-y-2">
<label class="text-sm font-medium">Nombre de la Categor�a *</label>
<Input
type="text"
